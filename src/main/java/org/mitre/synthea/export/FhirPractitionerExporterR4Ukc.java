...
public abstract class FhirPractitionerExporterR4Ukc {

  /**
   * Export the practitioner in FHIR R4 format.
   */
  public static void export(RandomNumberGenerator rand, long stop) {
    if (Config.getAsBoolean("exporter.practitioner.fhir_uk_core.export")) {
          ...
          for (String specialty : clinicians.keySet()) {
            ...
            for (Clinician doc : docs) {
              if (doc.getEncounterCount() > 0) {
                BundleEntryComponent entry = FhirR4Ukc.practitioner(bundle, doc);
...
//                }
      ...
      File outputFolder = Exporter.getOutputFolder("fhir-ukc", null);
